...
#include <vector>
    ...
    !defined(SIGNALSMITH_IGNORE_BROKEN_APPLECLANG)
...
// without `-ffast-math`
template <class V>
void complexMul(std::complex<V> *a, const std::complex<V> *b,
                const std::complex<V> *c, size_t size) {
  for (size_t i = 0; i < size; ++i) {
    auto bi = b[i], ci = c[i];
    a[i] = {bi.real() * ci.real() - bi.imag() * ci.imag(),
            bi.imag() * ci.real() + bi.real() * ci.imag()};
  }
}
template <class V>
void complexMulConj(std::complex<V> *a, const std::complex<V> *b,
                    const std::complex<V> *c, size_t size) {
  for (size_t i = 0; i < size; ++i) {
    auto bi = b[i], ci = c[i];
    a[i] = {bi.real() * ci.real() + bi.imag() * ci.imag(),
...
template <class V>
void complexMul(V *ar, V *ai, const V *br, const V *bi, const V *cr,
                const V *ci, size_t size) {
  for (size_t i = 0; i < size; ++i) {
...
template <class V>
void complexMulConj(V *ar, V *ai, const V *br, const V *bi, const V *cr,
                    const V *ci, size_t size) {
  for (size_t i = 0; i < size; ++i) {
...
template <size_t aStride, class V>
void interleaveCopy(const V *aReal, const V *aImag, V *bReal, V *bImag,
                    size_t bStride) {
  for (size_t bi = 0; bi < bStride; ++bi) {
...
template <typename Sample> struct SimpleFFT {
  using Complex = std::complex<Sample>;

  SimpleFFT(size_t size = 0) { resize(size); }
...
private:
  ...
  template <bool conjB> static Complex mul(const Complex &a, const Complex &b) {
    return conjB ? Complex{a.real() * b.real() + a.imag() * b.imag(),
                           a.imag() * b.real() - a.real() * b.imag()}
                 : Complex{a.real() * b.real() - a.imag() * b.imag(),
                           a.imag() * b.real() + a.real() * b.imag()};
  }

  // Calculate a [size]-point FFT, where each element is a block of [stride]
  // values
  template <bool inverse>
  void fftPass(size_t size, size_t stride, const Complex *input,
               Complex *output, Complex *working) {
    if (size / 4 > 1) {
  ...
  template <bool inverse>
  void combine4(size_t size, size_t stride, const Complex *input,
  ...
  template <bool inverse>
  void fftPass(size_t size, size_t stride, const Sample *inputR,
               const Sample *inputI, Sample *outputR, Sample *outputI,
               Sample *workingR, Sample *workingI) const {
    if (size / 4 > 1) {
      // Calculate four quarter-size FFTs
  ...
  // Combine interleaved results into a single spectrum
  template <bool inverse>
  void combine4(size_t size, size_t stride, const Sample *inputR,
                const Sample *inputI, Sample *outputR, Sample *outputI) const {
    auto twiddleStep = working.size() / size;
    for (size_t i = 0; i < size / 4; ++i) {
      Complex twiddleB = twiddles[i * twiddleStep];
      Complex twiddleC = twiddles[i * 2 * twiddleStep];
      ...
      const Sample *inputAr = inputR + 4 * i * stride,
                   *inputAi = inputI + 4 * i * stride;
      const Sample *inputBr = inputR + (4 * i + 1) * stride,
                   *inputBi = inputI + (4 * i + 1) * stride;
      const Sample *inputCr = inputR + (4 * i + 2) * stride,
                   *inputCi = inputI + (4 * i + 2) * stride;
      const Sample *inputDr = inputR + (4 * i + 3) * stride,
                   *inputDi = inputI + (4 * i + 3) * stride;
      Sample *outputAr = outputR + i * stride, *outputAi = outputI + i * stride;
      Sample *outputBr = outputR + (i + size / 4) * stride,
             *outputBi = outputI + (i + size / 4) * stride;
      Sample *outputCr = outputR + (i + size / 4 * 2) * stride,
             *outputCi = outputI + (i + size / 4 * 2) * stride;
      Sample *outputDr = outputR + (i + size / 4 * 3) * stride,
             *outputDi = outputI + (i + size / 4 * 3) * stride;
      for (size_t s = 0; s < stride; ++s) {
...
template <typename Sample> struct Pow2FFT {
  static constexpr bool prefersSplit =
      true; // whether this FFT implementation is faster when given
            // split-complex inputs
  using Complex = std::complex<Sample>;

  Pow2FFT(size_t size = 0) { resize(size); }
  // Allow move, but not copy
  Pow2FFT(const Pow2FFT &other) = delete;
  Pow2FFT(Pow2FFT &&other)
      : tmp(std::move(other.tmp)), simpleFFT(std::move(other.simpleFFT)) {}
  ...
  void fft(const Complex *time, Complex *freq) { simpleFFT.fft(time, freq); }
  void fft(const Sample *inR, const Sample *inI, Sample *outR, Sample *outI) {
    simpleFFT.fft(inR, inI, outR, outI);
  }

  void ifft(const Complex *freq, Complex *time) { simpleFFT.ifft(freq, time); }
  void ifft(const Sample *inR, const Sample *inI, Sample *outR, Sample *outI) {
...
template <typename Sample, bool splitComputation = false> struct SplitFFT {
  using Complex = std::complex<Sample>;
  ...
  SplitFFT(size_t size = 0) { resize(size); }
  ...
  void resize(size_t size) {
    ...
    while (!(outerSize & 1) &&
           (outerSize > maxSplit || innerSize < minInnerSize)) {
      innerSize *= 2;
      outerSize /= 2;
    }
    tmpFreq.resize(size);
    ...
    for (size_t i = 0; i < innerSize; ++i) {
      for (size_t s = 1; s < outerSize; ++s) {
        ...
        outerTwiddles[i + (s - 1) * innerSize] =
            std::polar(Sample(1), twiddlePhase);
      }
    }
    ...
    if (outerSize <= 1) {
      if (size > 0)
        plan.push_back(Step{StepType::passthrough, 0});
    } else {
  ...
  size_t size() const { return innerSize * outerSize; }
  size_t steps() const { return plan.size(); }
...
private:
  ...
  void fftStep(Step step, const Sample *inR, const Sample *inI, Sample *outR,
    ...
    case (StepType::twiddles): {
      ...
      if (inverse) {
        _impl::complexMulConj(outR + innerSize, outI + innerSize,
                              outR + innerSize, outI + innerSize, twiddlesR,
                              twiddlesI, innerSize * (outerSize - 1));
      } else {
        _impl::complexMul(outR + innerSize, outI + innerSize, outR + innerSize,
                          outI + innerSize, twiddlesR, twiddlesI,
                          innerSize * (outerSize - 1));
      }
      break;
    }
  ...
  template <bool inverse> void finalPass3(Complex *f0) {
    auto *f1 = f0 + innerSize;
    auto *f2 = f0 + innerSize * 2;
    const Complex tw1{Sample(-0.5),
                      Sample(-std::sqrt(0.75) * (inverse ? -1 : 1))};
    for (size_t i = 0; i < innerSize; ++i) {
      Complex a = f0[i], b = f1[i], c = f2[i];
      Complex bc0 = b + c, bc1 = b - c;
      f0[i] = a + bc0;
      f1[i] = {a.real() + bc0.real() * tw1.real() - bc1.imag() * tw1.imag(),
               a.imag() + bc0.imag() * tw1.real() + bc1.real() * tw1.imag()};
      f2[i] = {a.real() + bc0.real() * tw1.real() + bc1.imag() * tw1.imag(),
               a.imag() + bc0.imag() * tw1.real() - bc1.real() * tw1.imag()};
    }
  }
  template <bool inverse> void finalPass3(Sample *f0r, Sample *f0i) {
    auto *f1r = f0r + innerSize;
    auto *f1i = f0i + innerSize;
  ...
  template <bool inverse> void finalPass4(Complex *f0) {
    auto *f1 = f0 + innerSize;
    auto *f2 = f0 + innerSize * 2;
  ...
  template <bool inverse> void finalPass4(Sample *f0r, Sample *f0i) {
    auto *f1r = f0r + innerSize;
    auto *f1i = f0i + innerSize;
    auto *f2r = f0r + innerSize * 2;
    auto *f2i = f0i + innerSize * 2;
  ...
  template <bool inverse> void finalPass5(Complex *f0) {
    auto *f1 = f0 + innerSize;
    auto *f2 = f0 + innerSize * 2;
    auto *f3 = f0 + innerSize * 3;
    ...
    for (size_t i = 0; i < innerSize; ++i) {
      ...
      Complex be0 = b + e,
              be1 = {e.imag() - b.imag(), b.real() - e.real()}; // (b - e)*i
  ...
  template <bool inverse> void finalPass5(Sample *f0r, Sample *f0i) {
    auto *f1r = f0r + innerSize;
    auto *f1i = f0i + innerSize;
    auto *f2r = f0r + innerSize * 2;
    auto *f2i = f0i + innerSize * 2;
    ...
    for (size_t i = 0; i < innerSize; ++i) {
      Sample ar = f0r[i], ai = f0i[i], br = f1r[i], bi = f1i[i], cr = f2r[i],
             ci = f2i[i], dr = f3r[i], di = f3i[i], er = f4r[i], ei = f4i[i];
      ...
      Sample bcde01r = be0r * tw1r + cd0r * tw2r,
             bcde01i = be0i * tw1r + cd0i * tw2r;
      Sample bcde02r = be0r * tw2r + cd0r * tw1r,
             bcde02i = be0i * tw2r + cd0i * tw1r;
      Sample bcde11r = be1r * tw1i + cd1r * tw2i,
             bcde11i = be1i * tw1i + cd1i * tw2i;
      Sample bcde12r = be1r * tw2i - cd1r * tw1i,
             bcde12i = be1i * tw2i - cd1i * tw1i;
  ...
  template <bool inverse> void finalPassN(Complex *f0) {
    for (size_t i = 0; i < innerSize; ++i) {
      ...
      for (size_t f = 1; f < outerSize; ++f) {
        Complex frequencySum = dftTmp[0];

        for (size_t i2 = 1; i2 < outerSize; ++i2) {
          size_t twistIndex = (i2 * f) % outerSize;
          Complex twist = inverse ? std::conj(dftTwists[twistIndex])
                                  : dftTwists[twistIndex];
          frequencySum += Complex{dftTmp[i2].real() * twist.real() -
                                      dftTmp[i2].imag() * twist.imag(),
                                  dftTmp[i2].imag() * twist.real() +
                                      dftTmp[i2].real() * twist.imag()};
        }

        offsetFreq[f * innerSize] = frequencySum;
      }
    }
  }
  template <bool inverse> void finalPassN(Sample *f0r, Sample *f0i) {
    ...
    for (size_t i = 0; i < innerSize; ++i) {
      ...
      for (size_t f = 1; f < outerSize; ++f) {
        Sample frequencySumR = *tmpR, frequencySumI = *tmpI;
        ...
        for (size_t i2 = 1; i2 < outerSize; ++i2) {
          ...
          Complex twist = inverse ? std::conj(dftTwists[twistIndex])
                                  : dftTwists[twistIndex];
          frequencySumR +=
              tmpR[i2] * twist.real() - tmpI[i2] * twist.imag();
          frequencySumI +=
              tmpI[i2] * twist.real() + tmpR[i2] * twist.imag();
        }

        offsetR[f * innerSize] = frequencySumR;
        offsetI[f * innerSize] = frequencySumI;
      }
    }
  }
};
...
struct SimpleRealFFT {
  ...
  SimpleRealFFT(size_t size = 0) { resize(size); }
  ...
  void fft(const Sample *time, Complex *freq) {
    ...
    freq[0] = {tmpFreq[0].real(), tmpFreq[tmpFreq.size() / 2].real()};
  }
  void fft(const Sample *inR, Sample *outR, Sample *outI) {
    Sample *tmpFreqR = (Sample *)tmpFreq.data(),
           *tmpFreqI = tmpFreqR + tmpFreq.size();
...
/// implementations where available
template <typename Sample> struct Pow2RealFFT : public SimpleRealFFT<Sample> {
...
/// chunks
template <typename Sample, bool splitComputation = false,
          bool halfBinShift = false>
struct RealFFT {
  using Complex = std::complex<Sample>;
  static constexpr bool prefersSplit =
      SplitFFT<Sample, splitComputation>::prefersSplit;

  static size_t fastSizeAbove(size_t size) {
    return ComplexFFT::fastSizeAbove((size + 1) / 2) * 2;
  }

  RealFFT(size_t size = 0) { resize(size); }
      ...
      for (size_t i = 0; i < twiddles.size(); ++i) {
        Sample rotPhase = static_cast<Sample>(
            static_cast<double>(i) *
                (-2.0 * M_PI / static_cast<double>(size)) -
            M_PI / 2.0); // bake rotation by (-i) into twiddles
    ...
    if (complexPrefersSplit) {
      ...
      if (step-- == 0) {
        ...
        for (size_t i = startI; i < endI; ++i) {
          ...
          Sample evenRotMinusIR =
              evenIR * twiddle.real() - evenII * twiddle.imag();
          Sample evenRotMinusII =
              evenII * twiddle.real() + evenIR * twiddle.imag();
      ...
      if (step-- == 0) {
        if (halfBinShift) {
          for (size_t i = 0; i < hSize; ++i) {
            Sample tr = time[2 * i], ti = time[2 * i + 1];
            Complex twist = halfBinTwists[i];
            tmpTime[i] = {tr * twist.real() - ti * twist.imag(),
        ...
        complexFft.fft(step,
                       canUseTime ? (const Complex *)time : tmpTime.data(),
                       tmpFreq.data());
      } else {
        if (!halfBinShift) {
          Complex bin0 = tmpFreq[0];
          freq[0] = {// pack DC & Nyquist together
                     bin0.real() + bin0.imag(), bin0.real() - bin0.imag()};
        }
        ...
        for (size_t i = startI; i < endI; ++i) {
          ...
          Complex evenI =
              (tmpFreq[i] - std::conj(tmpFreq[conjI])) * Sample(0.5);
          ...
          freq[conjI] = {odd.real() - evenRotMinusI.real(),
                         evenRotMinusI.imag() - odd.imag()};
        }
      }
    }
  }
  ...
  void ifft(size_t step, const Complex *freq, Sample *time) {
    if (complexPrefersSplit) {
      ...
      if (splitFirst || step-- == 0) {
        ...
        for (size_t i = startI; i < endI; ++i) {
          ...
          Complex evenI = {// Conjugate twiddle
                           evenRotMinusI.real() * twiddle.real() +
                               evenRotMinusI.imag() * twiddle.imag(),
                           evenRotMinusI.imag() * twiddle.real() -
                               evenRotMinusI.real() * twiddle.imag()};
      ...
      if (splitFirst || step-- == 0) {
        Complex bin0 = freq[0];
        if (!halfBinShift) {
          tmpFreq[0] = {bin0.real() + bin0.imag(), bin0.real() - bin0.imag()};
        }
        size_t hSize = complexFft.size();
        size_t startI = halfBinShift ? 0 : 1;
        size_t endI = hSize / 2 + 1;
        ...
        for (size_t i = startI; i < endI; ++i) {
          ...
          tmpFreq[i] = odd + evenI;
          tmpFreq[conjI] = {odd.real() - evenI.real(),
                            evenI.imag() - odd.imag()};
        }
      } else if (step < complexFft.steps()) {
        // Can't just use time as (Complex *), since it might not be aligned
        // properly
        complexFft.ifft(step, tmpFreq.data(),
                        canUseTime ? (Complex *)time : tmpTime.data());
      } else {
        size_t hSize = complexFft.size();
        if (halfBinShift) {
          for (size_t i = 0; i < hSize; ++i) {
