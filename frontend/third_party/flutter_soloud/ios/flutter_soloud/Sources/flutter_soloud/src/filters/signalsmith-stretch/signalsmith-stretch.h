...
struct SignalsmithStretch {
  ...
  void presetDefault(int nChannels, Sample sampleRate,
    ...
    configure(nChannels,
              static_cast<int>(std::round(
                  sampleRate * static_cast<Sample>(0.12))),
              static_cast<int>(std::round(
                  sampleRate * static_cast<Sample>(0.03))),
              splitComputation);
  }
  void presetCheaper(int nChannels, Sample sampleRate,
    ...
    configure(nChannels,
              static_cast<int>(std::round(
                  sampleRate * static_cast<Sample>(0.1))),
              static_cast<int>(std::round(
                  sampleRate * static_cast<Sample>(0.04))),
              splitComputation);
  }
  ...
  bool splitComputation() const { return _splitComputation; }
  ...
  void setTransposeFactor(Sample multiplier, Sample tonalityLimit = 0) {
    ...
    if (tonalityLimit > 0) {
      freqTonalityLimit =
          tonalityLimit /
          std::sqrt(multiplier); // compromise between input and output limits
    } else {
      freqTonalityLimit = 1;
    }
  ...
  // copies intput to a buffer. You should ideally feed it `seekLength()` frames
  // of input, unless it's directly after a `.reset()` (in which case
  ...
  void seek(Inputs &&inputs, int inputSamples, double playbackRate) {
    ...
    int startIndex = std::max<int>(
        0,
        inputSamples - int(tmpProcessBuffer.size())); // start position in input
    int padStart = int(tmpProcessBuffer.size() + startIndex) -
                   inputSamples; // start position in tmpProcessBuffer
    ...
    didSeek = true;
    seekTimeFactor = (playbackRate * stft.defaultInterval() > 1)
                         ? 1 / playbackRate
                         : stft.defaultInterval();
  }
  ...
  template <class Inputs> void outputSeek(Inputs &&inputs, int inputLength) {
    // TODO: add fade-out parameter to avoid clicks, instead of doing a full
    // reset
    reset();
    // Assume we've been handed enough surplus input to produce
    // `outputLatency()` samples of pre-roll
    ...
    struct BufferOutput {
      ...
      Sample *operator[](int c) { return samples + c * length; }
  ...
  void process(Inputs &&inputs, int inputSamples, Outputs &&outputs,
    ...
    auto copyInput = [&](int toIndex) {
      int length =
          std::min<int>(int(stft.blockSamples() + stft.defaultInterval()),
                        toIndex - prevCopiedInput);
    ...
    if (totalEnergy < noiseFloor) {
      if (silenceCounter >= 2 * stft.blockSamples()) {
        ...
        if (inputSamples > 0) {
          ...
          for (int outputIndex = 0; outputIndex < outputSamples;
               ++outputIndex) {
    ...
    for (int outputIndex = 0; outputIndex < outputSamples; ++outputIndex) {
      ...
      if (newBlock) {
        blockProcess.step = 0;
        blockProcess.steps =
            0; // how many processing steps this block will have
        blockProcess.samplesSinceLast = 0;

        // Time to process a spectrum!  Where should it come from in the input?
        int inputOffset = static_cast<int>(std::round(
            Sample(outputIndex) * Sample(inputSamples) /
            Sample(outputSamples)));
        int inputInterval = inputOffset - prevInputOffset;
        prevInputOffset = inputOffset;

        copyInput(inputOffset);
        stashedInput = stft.input; // save the input state, since that's what
                                   // we'll analyse later
        if (_splitComputation) {
          stashedOutput =
              stft.output; // save the current output, and read from it
          stft.moveOutput(
              stft.defaultInterval()); // the actual input jumps forward in time
                                       // by one interval, ready for the
                                       // synthesis
        }
        ...
        if (blockProcess.newSpectrum) {
          ...
          // (give or take 1 sample)
          blockProcess.reanalysePrev =
              didSeek ||
              std::abs(inputInterval - int(stft.defaultInterval())) > 1;
        ...
        blockProcess.processFormants =
            formantMultiplier != 1 ||
            (formantCompensation && blockProcess.mappedFrequencies);

        blockProcess.timeFactor =
            didSeek
                ? seekTimeFactor
                : Sample(stft.defaultInterval()) /
                      std::max<Sample>(Sample(1), Sample(inputInterval));
        didSeek = false;
      ...
      if (_splitComputation) {
        Sample processRatio =
            Sample(blockProcess.samplesSinceLast + 1) / stft.defaultInterval();
        const size_t proportionalStep = static_cast<size_t>(std::ceil(
            Sample(blockProcess.steps) * processRatio));
        processToStep =
            std::min<size_t>(blockProcess.steps, proportionalStep);
      }
      ...
      while (blockProcess.step < processToStep) {
        ...
        if (blockProcess.newSpectrum) {
          if (blockProcess.reanalysePrev) {
            ...
            if (step < 1) {
              ...
              for (int c = 0; c < channels; ++c) {
                auto channelBandView = bandsForChannel(c);
                auto *spectrumBands = stft.spectrum(c);
                for (int b = 0; b < bands; ++b) {
                  channelBandView[b].prevInput = spectrumBands[b];
                }
              }
              continue;
            }
          ...
          if (step < 1) {
            ...
            for (int c = 0; c < channels; ++c) {
              ...
              auto *spectrumBands = stft.spectrum(c);
              for (int b = 0; b < bands; ++b) {
                channelBandView[b].input = spectrumBands[b];
              }
            }
            continue;
          }
        ...
        if (step < 1) {
          ...
          for (int c = 0; c < channels; ++c) {
            ...
            auto *spectrumBands = stft.spectrum(c);
            for (int b = 0; b < bands; ++b) {
              spectrumBands[b] = channelBandView[b].output;
            }
          }
          continue;
        }
  ...
  // Read the remaining output, providing no further input.  If `outputSamples`
  // is more than one interval, it will compute additional blocks assuming a
  // zero-valued input
  template <class Outputs>
  void flush(Outputs &&outputs, int outputSamples, Sample playbackRate = 0) {
    struct Zeros {
      struct Channel {
        Sample operator[](int) { return 0; }
      };
      Channel operator[](int) { return {}; }
    } zeros;
    // If we're asked for more than an interval of extra output, then zero-pad
    // the input
    int outputBlock = std::max<int>(0, outputSamples - stft.defaultInterval());
    if (outputBlock > 0)
      process(zeros, outputBlock * playbackRate, outputs, outputBlock);
  ...
  template <class Inputs, class Outputs>
  bool exact(Inputs &&inputs, int inputSamples, Outputs &&outputs,
             int outputSamples) {
...
private:
  ...
  static constexpr Sample maxCleanStretch{
      2}; // time-stretch ratio before we start randomising phases
  ...
  Sample bandToFreq(Sample b) const { return stft.binToFreq(b); }
  Sample freqToBand(Sample f) const { return stft.freqToBin(f); }
  ...
  template <Complex Band::*member> Complex getBand(int channel, int index) {
  ...
  template <Sample Band::*member> Sample getBand(int channel, int index) {
    if (index < 0 || index >= bands)
      return 0;
    return channelBands[index + channel * bands].*member;
  }
  ...
  using RandomEngineImpl =
      typename std::conditional<std::is_void<RandomEngine>::value,
                                std::default_random_engine, RandomEngine>::type;
  RandomEngineImpl randomEngine;

  size_t processSpectrumSteps = 0;
  static constexpr size_t splitMainPrediction =
      8; // it's just heavy, since we're blending up to 4 different phase
         // predictions
  void updateProcessSpectrumSteps() {
    ...
    if (blockProcess.newSpectrum)
      processSpectrumSteps += 1; // .input -> .prevInput
    if (blockProcess.processFormants)
      processSpectrumSteps += 3;
  }
  void processSpectrum(size_t step) {
    ...
    if (blockProcess.newSpectrum) {
      if (step < size_t(channels)) {
        ...
        Complex rot =
            std::polar(Sample(1), bandToFreq(0) * stft.defaultInterval() *
                                      Sample(2 * M_PI));
        Sample freqStep = bandToFreq(1) - bandToFreq(0);
        Complex rotStep = std::polar(
            Sample(1), freqStep * stft.defaultInterval() * Sample(2 * M_PI));
    ...
    if (step < size_t(channels)) {
      ...
      for (int b = 0; b < bands; ++b) {
        ...
        prediction.energy =
            getFractional<&Band::inputEnergy>(c, lowIndex, fracIndex);
        prediction.energy *= std::max<Sample>(
            0, mapPoint.freqGrad); // scale the energy according to local
                                   // stretch factor
        prediction.input = getFractional<&Band::input>(c, lowIndex, fracIndex);

        auto &outputBin = bins[b];
        Complex prevInput =
            getFractional<&Band::prevInput>(c, lowIndex, fracIndex);
        Complex freqTwist = _impl::mul<true>(prediction.input, prevInput);
        Complex phase = _impl::mul(outputBin.output, freqTwist);
        outputBin.output =
            phase / (std::max(prevEnergy, prediction.energy) + noiseFloor);
      }
      return;
    }
    step -= channels;
    ...
    if (step < splitMainPrediction) {
      ...
      for (int b = startB; b < endB; ++b) {
        ...
        if (b > 0) {
          Sample binTimeFactor =
              randomTimeFactor ? timeFactorDist(randomEngine) : timeFactor;
          Complex downInput = getFractional<&Band::input>(
              maxChannel, mapPoint.inputBin - binTimeFactor);
          Complex shortVerticalTwist =
              _impl::mul<true>(prediction.input, downInput);

          auto &downBin = bins[b - 1];
          phase += _impl::mul(downBin.output, shortVerticalTwist);

          if (b >= longVerticalStep) {
            Complex longDownInput = getFractional<&Band::input>(
                maxChannel,
                mapPoint.inputBin - longVerticalStep * binTimeFactor);
            Complex longVerticalTwist =
                _impl::mul<true>(prediction.input, longDownInput);
        ...
        if (b < bands - 1) {
          ...
          Sample binTimeFactor =
              randomTimeFactor ? timeFactorDist(randomEngine) : timeFactor;
          Complex downInput = getFractional<&Band::input>(
              maxChannel, upMapPoint.inputBin - binTimeFactor);
          Complex shortVerticalTwist =
              _impl::mul<true>(upPrediction.input, downInput);
          ...
          if (b < bands - longVerticalStep) {
            ...
            Complex longDownInput = getFractional<&Band::input>(
                maxChannel,
                longUpMapPoint.inputBin - longVerticalStep * binTimeFactor);
            Complex longVerticalTwist =
                _impl::mul<true>(longUpPrediction.input, longDownInput);
        ...
        for (int c = 0; c < channels; ++c) {
          if (c != maxChannel) {
            ...
            Complex channelTwist =
                _impl::mul<true>(channelPrediction.input, prediction.input);
  ...
  Sample estimateFrequency() {
    ...
    for (int b = 1; b < bands - 1; ++b) {
      ...
      // local maxima only
      if (e < formantMetric[b - 1] || e <= formantMetric[b + 1])
        continue;
    ...
    if (formantMetric[peakIndices[1]] > formantMetric[peakIndices[2]] * 0.1) {
      ...
      if (diff > peakEstimate / 8 && diff < peakEstimate * 7 / 8)
        peakEstimate = peakEstimate % diff;
      if (formantMetric[peakIndices[0]] >
          formantMetric[peakIndices[2]] * 0.01) {
        int secondaryDiff = std::abs(peakEstimate - peakIndices[0]);
        if (secondaryDiff > peakEstimate / 8 &&
            secondaryDiff < peakEstimate * 7 / 8)
          peakEstimate = peakEstimate % secondaryDiff;
      }
    }
  ...
  void updateFormants(size_t step) {
    if (step-- == 0) {
      ...
      for (int b = 0; b < bands; ++b) {
        ...
        for (int c = 0; c < channels; ++c) {
          Band *bins = bandsForChannel(c);
          // This is what's used to decide the output energy, so this affects
          // the output
  ...
  template <class Io> struct OffsetIO {
    Io &io;
    int offset;
    ...
    Channel operator[](int c) { return {io, c, offset}; }
  };
};
