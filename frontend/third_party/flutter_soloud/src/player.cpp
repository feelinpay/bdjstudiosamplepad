...
std::vector<PlaybackDevice> Player::listPlaybackDevices()
{
    ...
    for (ma_uint32 i = 0; i < playbackCount; i++)
    {
        ...
        PlaybackDevice cd;
        cd.name = soloudDuplicateString(pPlaybackInfos[i].name);
...
PlayerErrors Player::getStreamTimeConsumed(unsigned int hash, float *timeConsumed)
{
    ...
    *timeConsumed = static_cast<float>(
        static_cast<SoLoud::BufferStream *>(s->sound.get())->getStreamTimeConsumed());
    return PlayerErrors::noError;
}
...
unsigned int Player::getActiveVoiceCount_internal()
{
    ...
    for (auto &s : sounds)
    {
        count += static_cast<unsigned int>(s->handle.size());
    }
    return count;
}
...
int Player::countAudioSource(unsigned int soundHash)
{
    ...
    case TYPE_SYNTH:
        return 0;
    case TYPE_WAV:
        return soloud.countAudioSource(*static_cast<SoLoud::Wav *>(s->sound.get()));
    case TYPE_WAVSTREAM:
        return soloud.countAudioSource(*static_cast<SoLoud::WavStream *>(s->sound.get()));
    case TYPE_BUFFER_STREAM:
        return soloud.countAudioSource(*static_cast<SoLoud::BufferStream *>(s->sound.get()));
    default:
        return 0;
    }
}
    ...
    double loopingStartAt)
{
    ...
    if (looping)
    {
        seek(newHandle, static_cast<float>(loopingStartAt));
        setLoopPoint(newHandle, loopingStartAt);
        setLooping(newHandle, true);
